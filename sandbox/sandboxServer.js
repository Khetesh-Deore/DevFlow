...
const LANGUAGE_CONFIG = {
  cpp: {
    extension: '.cpp',
    compile: (file, output) => `g++ -std=c++17 -O2 -Wall "${file}" -o "${output}"`,
    execute: (file) => `"${file}"`,
    needsCompile: true
  },
  c: {
    extension: '.c',
    compile: (file, output) => `gcc -std=c11 -O2 -Wall "${file}" -o "${output}"`,
    execute: (file) => `"${file}"`,
    needsCompile: true
  },
  java: {
...
const executeCode = (language, filePath, input, timeout) => {
...
const runSingleTestCase = async (language, code, testcase, constraints) => {
  await ensureTempDir();
